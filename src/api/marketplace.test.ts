...
const publicGet = vi.mocked(publicApiClient.get)
...
describe('marketplaceApi', () => {
  ...
  it('calls the spec public listings path with all compacted params', async () => {
    ...
    expect(publicGet).toHaveBeenCalledWith('/marketplace/listings', {
      params: {
        q: 'фара',
        city: 'Львів',
        make: 'Audi',
  ...
  it('fetches listing detail by slug', async () => {
    ...
    expect(publicGet).toHaveBeenCalledWith('/marketplace/listings/fara-1')
