...
export const marketplaceApi = {
  async getCatalog(
    ...
    try {
      const resp = await publicApiClient.get<
        PagedResult<MarketplaceListingCardDto>
  ...
  async getListing(slugOrId: string): Promise<MarketplaceListingDetailDto> {
    const resp = await publicApiClient.get<MarketplaceListingDetailDto>(
  ...
  async getShop(slug: string): Promise<MarketplaceShopPublicDto> {
    const resp = await publicApiClient.get<MarketplaceShopPublicDto>(
  ...
  async getSellerShop(): Promise<MarketplaceShopDto | null> {
    const resp = await apiClient.get<MarketplaceShopDto | null>(
      '/marketplace/shop',
    )
    return resp.data
  },
  async upsertSellerShop(
    payload: UpsertMarketplaceShopRequest,
  ): Promise<MarketplaceShopDto> {
    const resp = await apiClient.put<MarketplaceShopDto>(
      '/marketplace/shop',
      payload,
    )
    return resp.data
  ...
  async getSellerSummary(): Promise<MarketplaceSellerSummaryDto> {
    const resp = await apiClient.get<MarketplaceSellerSummaryDto>(
  ...
  async getSellerListings(
    page = 1,
    pageSize = 30,
  ): Promise<PagedResult<MarketplaceSellerListingDto>> {
    const resp = await apiClient.get<PagedResult<MarketplaceSellerListingDto>>(
      '/marketplace/seller/listings',
      { params: { page, per_page: pageSize } },
    )
    return resp.data
  },
  async searchSellerParts(
    q?: string,
    page = 1,
    pageSize = 30,
  ): Promise<PagedResult<MarketplaceSellerPartDto>> {
    const resp = await apiClient.get<PagedResult<MarketplaceSellerPartDto>>(
      '/marketplace/seller/parts',
      { params: { q, page, per_page: pageSize } },
    )
    return resp.data
  },
  async createListingFromPart(
    partId: string,
  ): Promise<MarketplaceSellerListingDto> {
    const resp = await apiClient.post<MarketplaceSellerListingDto>(
      `/marketplace/seller/listings/from-part/${partId}`,
    )
    return resp.data
  },
  async updateListing(
    id: string,
    payload: UpdateMarketplaceListingRequest,
  ): Promise<MarketplaceSellerListingDto> {
    const resp = await apiClient.patch<MarketplaceSellerListingDto>(
      `/marketplace/seller/listings/${id}`,
      payload,
    )
    return resp.data
  },
